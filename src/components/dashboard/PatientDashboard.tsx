...
import { Users, Search, MapPin, Clock, Heart, AlertCircle, Phone } from "lucide-react";
...
export const PatientDashboard = () => {
  ...
  const activeRequests = [
    { id: 1, bloodType: 'A+', units: 2, urgency: 'Critical', hospital: 'City General Hospital', created: '2024-01-15' },
    { id: 2, bloodType: 'A+', units: 1, urgency: 'Medium', hospital: 'Metro Medical Center', created: '2024-01-10' },
  ];

  const availableDonors = [
    { id: 1, bloodType: 'A+', location: '2.5 km away', lastDonation: '3 months ago', status: 'Available' },
    { id: 2, bloodType: 'A+', location: '4.1 km away', lastDonation: '1 month ago', status: 'Available' },
    { id: 3, bloodType: 'A+', location: '5.8 km away', lastDonation: '2 weeks ago', status: 'Recently Donated' },
  ];
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-600">Blood Type Needed</label>
              <BloodTypeSelector value={bloodTypeNeeded} onChange={setBloodTypeNeeded} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-gray-600">Units Required</label>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                { name: 'City General Hospital', distance: '1.2 km', bloodBank: true, emergency: true },
                { name: 'Metro Medical Center', distance: '2.8 km', bloodBank: true, emergency: false },
                { name: 'Community Blood Bank', distance: '3.5 km', bloodBank: true, emergency: false },
              ].map((hospital, index) => (
          ...
          <div className="space-y-4">
            {availableDonors.map((donor) => (
              <div key={donor.id} className="border rounded-lg p-4">
                ...
                <div className="flex gap-2">
                  <Button size="sm" disabled={donor.status !== 'Available'}>
                    Contact Donor
                  </Button>
                  <Button size="sm" variant="outline">
                    <MapPin className="mr-1 h-3 w-3" />
